...
// 루트 문자열 하나가 "모호하지 않은" 절대경로인지 판정한다(플랫폼별 기준). 상대경로·빈 문자열은
// 물론, 윈도우에서 드라이브 문자·UNC 없이 구분자로만 시작하는 경로(예: "/workspace")도 거부한다 —
// path.isAbsolute 만으로는 이런 경로가 걸러지지 않는데, runtime 에 process.cwd() 의 드라이브가
// 바뀌면 예상치 못한 드라이브의 폴더를 노출할 수 있기 때문이다(POSIX 는 드라이브 개념이 없어 '/' 로
// 시작하면 그 자체로 모호하지 않다). checkPath(아래)뿐 아니라 config 로드 시점의 WORKER_ROOTS 검증
// (config.ts 의 loadWorkerConfig)도 이 판정을 그대로 재사용한다 — 두 곳의 기준이 갈리면, config
// 로드는 통과했는데 실제 호출은 이 checkPath 가 전부 거부하는 워커가 조용히 뜨게 된다.
export function isUnambiguousRoot(root: string): boolean {
  return process.platform === "win32" ? hasUnambiguousWindowsRoot(root) : isAbsolute(root);
}
...
export function checkPath(target: string, roots: string[]): PathCheck {
  ...
  // 잘못된 루트를 조용히 필터링하지 않고 전체를 거부해야 워커의 루트 설정이 부분적으로 잘못된
  // 경우 명확히 드러난다.
  for (const root of roots) {
    if (!isUnambiguousRoot(root)) {
      return {
        ok: false,
