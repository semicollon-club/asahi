...
// 소유자 전용 도구(manage_access·allow_dir 등)는 여기 넣지 않는다 — 손님에게는 못 쓰는 기능을
// 알려줘봤자 혼란만 준다. db_query 는 2026-09-17 부터 손님도 쓸 수 있지만(ADR 0010) 여기 넣지
// 않는다 — 이 목록은 "말로 시키면 되는 것"을 알리는 자리이고, DB 조회는 사람이 먼저 떠올려
// 부탁하는 종류가 아니라 아사히가 답을 실측으로 확인할 때 스스로 쓰는 도구다(persona 의 안내가
// 그 자리다).
//
...
const GUEST_TIPS = [
  ...
  // 파일 반환(2026-09-05): 같은 이유로 한 줄 — 만든 파일이 디스코드로 돌아온다는 사실 자체를 알린다.
