...
// 2026-08-07 첫 실사용: 소유자가 폴더를 지명해 발행했더니 부원 폴더 전체(개인정보 파일 포함)가
// 부원의 디스코드 ID 를 이름으로 올라갔다. 손님은 scopeDirs 가 좁혀 워크스페이스 전체를 지정할
// 수는 없지만, 모델이 구조를 모르면 프로젝트 폴더를 안 만들고 작업 폴더에 파일을 흩어 놓아
// 같은 혼동이 생긴다. 안내가 그 구조를 먼저 못박게 한다.
describe("발행 안내가 폴더 구조를 못박는가", () => {
  const p = () => buildSystemPrompt({ role: "allowed", isPrivate: false, isOwner: false, workerConnected: true, githubReady: true });

  it("프로젝트는 작업 폴더 밑 폴더 하나라는 것을 말한다", () => {
    expect(p()).toContain("프로젝트는 작업 폴더 바로 밑에 폴더 하나로 만듭니다");
  });

  it("작업 폴더 자체나 남의 폴더를 발행하지 말라고 말한다", () => {
    expect(p()).toContain("작업 폴더 자체나 남의 폴더를 발행하려 하지 마세요");
  });

  // 이름 규칙을 모르면 한글 폴더를 발행하려다 normalizeRepoName 이 거부하는 이유를 모른 채
  // 사용자에게 "왜 안 되는지" 를 설명하지 못한다.
  it("이름 규칙과 한글·공백 폴더가 안 된다는 것을 말한다", () => {
    expect(p()).toContain("영문·숫자·하이픈·밑줄");
