...
describe("AllowedDirsRepo", () => {
  ...
  it("add 하면 정규화된 절대경로로 저장되고 list 에 나타난다", async () => {
    ...
    expect(await repo.list("owner-laptop")).toEqual([path.resolve("C:\\proj\\a")]);
  });
  ...
  it("여러 폴더를 add 하면 모두 누적된다", async () => {
    ...
    expect((await repo.list("owner-laptop")).sort()).toEqual([path.resolve("C:\\proj\\a"), path.resolve("C:\\proj\\b")].sort());
  });
  ...
  it("remove 하면 목록에서 제거된다", async () => {
    ...
    expect(await repo.list("owner-laptop")).toEqual([path.resolve("C:\\proj\\b")]);
  });
  ...
  it("서로 다른 워커는 서로 격리된다(한쪽 add/remove 가 다른쪽에 영향 없음)", async () => {
    ...
    expect(await repo.list("owner-laptop")).toEqual([path.resolve("C:\\proj\\a")]);
    expect(await repo.list("semicolon-shared")).toEqual([path.resolve("C:\\proj\\b")]);

    await repo.remove("owner-laptop", "C:\\proj\\a");
    expect(await repo.list("owner-laptop")).toEqual([]);
    expect(await repo.list("semicolon-shared")).toEqual([path.resolve("C:\\proj\\b")]); // 영향 없음
  });
  ...
  it("같은 경로라도 워커가 다르면 독립적으로 저장된다", async () => {
    ...
    expect(await repo.list("owner-laptop")).toEqual([path.resolve("C:\\proj\\shared")]);
    expect(await repo.list("semicolon-shared")).toEqual([path.resolve("C:\\proj\\shared")]);
