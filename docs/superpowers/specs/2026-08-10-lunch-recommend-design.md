...
**사람이 지켜보지 않는 턴은 자동으로 닫힌다 — 다만 두 턴이 닫히는 이유는 서로 다르다
(2026-08-14 문서 정합, Important 1 리뷰 후속으로 정정).** 정기 게시(`digest.ts`)는 신원 자체
(`isOwner:false, isPrivate:false` 로 고정)가 위 게이팅(`isOwner && isPrivate`)을 통과하지
못해 애초에 후보가 아니다. 유휴 요약(`writeSummary`)은 다르다 — **대화 주인이 소유자 자신이면
그 요약 턴의 신원도 `isOwner && isPrivate` 를 실제로 만족한다**(소유자 DM 이 유휴해지는
경우이므로, 이 신원 게이팅 자체로는 걸러지지 않는다). 그 경우를 닫는 것은 신원이 아니라 턴별
강제 폐쇄 축 `noLunchTools` 다 — `noRemoteTools`/`noWebTools`/`noMemoryWrite` 와 같은 자리이고,
`writeSummary` 가 요청을 만들 때 세운다(`agent/src/core/core.ts`). `config.lunch` 자체는
건드리지 않으므로 평상시 턴에는 영향이 없다. (한때 이 문장은 "유휴 요약·정기 게시는 소유자
DM 이 아니다" 라고만 적혀 있었는데, 소유자 자신의 DM 이 유휴 요약되는 경우에는 그 자체가
틀린 서술이었다 — 리뷰가 실측으로 확인했다.)
