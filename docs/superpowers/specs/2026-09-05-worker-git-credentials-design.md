...
봇과 워커는 같은 브랜치(`production`)에서 나가지만 따로 배포된다(봇은 Railway, 워커는 5분 주기
자동 갱신 — 이 기능을 처음 배포한 2026-09-05 당일까지는 워커가 `main` 을 따랐고, 같은 날 production
추적으로 바꿨다). 어느 쪽이 먼저 새 코드가 되어도 안전하다 — 옛 워커는 `git` 인자를 무시하고(자격증명
없이 예전처럼 실패), 옛 봇을 만난 새 워커는 프롬프트만 끄고 기본 사유를 말한다.
