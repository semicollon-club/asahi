...
**상태(2026-09-06): 2.1~2.7 코드·문서 완료, 실사용 검증 대기(`deploy/smoke-test.md` 2단계 항목 다섯).** 계획과 다른 점 — (1) 2.5 의
소유자 DM 폴백은 `workerSelect.ts` 의 선택자를 바꾸지 않고 `agent.ts` 의 `resolveTurnWorker` 에서 "개인 워커가 없거나 끊겨 있으면
공유 워커" 로 했다(선택자는 여전히 "소유자 DM 은 개인 워커"를 말하고, 실제로 붙어 있으면 그것이 우선이다). (2) 하네스 턴의
시스템 프롬프트는 core.ts 가 만든 것(원격·기억 도구 안내)이 아니라 `agent.ts` 가 `buildSystemPrompt({ harness })` 로 다시 만든다 —
그 턴에는 봇의 MCP 도구가 없기 때문이다(4단계까지). (3) 이미지가 붙은 턴은 새 경로를 타지 않는다(base64 가 프레임 상한을
넘을 수 있다). (4) `progressFromMessage` 를 `core/sdkEvents.ts` 로 떼어 봇·워커가 같은 매핑을 쓴다.
...
## 3단계 — 프록시 운영화 — **구현 완료(2026-09-06)**

| # | 태스크 | 완료 기준 | 상태 |
|---|---|---|---|
| 3.1 | 모델 고정(본문 `model` = 프로필 모델) | 다른 모델 요청은 400 + 기록 | ✅ 작업 토큰에 모델 클레임(`jobToken.ts`), 프록시가 본문 model 을 고정(`llmProxy.ts` — 불일치 400, 업스트림 미전송) |
| 3.2 | 사용량 표 `llm_usage`(작업·부원·입력/출력 토큰·모델·시각), `runtime_info`·`db_query` 에서 조회 | 턴마다 행 하나 | ✅ `llm_usage` 표·`llmUsageRepo`·프록시 SSE 스니퍼(모델 호출당 한 행). **조회는 `db_query` 로**(runtime_info 통합은 후속 — 배선 최소화) |
| 3.3 | 부원별 창 상한(토큰·턴) — `turns.reserve` 옆에. 429 → 한국어 사유, 소유자 우선 | 초과·한도가 사용자 문구로 | ✅ 사전 게이트(`core.ts` — 창 안 토큰 합 ≥ 상한이면 손님 거절, 소유자 면제). 업스트림 429 는 통과 → 봇 턴 실패 안내 |
| 3.4 | 장애 폴백: 러너 없음·프록시 오류 → 봇 쪽 도구 없는 턴 + 사유 | 워커를 꺼도 대화는 된다 | ✅ 워커 off·비하네스 → `decideHarnessDispatch` 거짓 → 봇 자기 세션(테스트됨). 프록시·러너 장애는 사유 담은 오류 안내(자동 재실행은 이중 과금 우려로 보류) |
