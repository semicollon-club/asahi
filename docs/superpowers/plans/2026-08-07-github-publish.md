...
- **커밋은 `CONTRIBUTING.md` "커밋 규약" 을 따른다** — 접두사 붙은 제목 + **한국어 본문("무엇을"이 아니라 "왜")** + 말미에 `Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>`. 아래 각 태스크의 커밋 단계는 제목만 예시로 적어 두었다. **그 한 줄을 그대로 실행하지 말고** 본문과 트레일러를 채워 넣어라. (Task 1 검토에서 실제로 이 규약을 어긴 커밋이 나왔다 — 예시를 문자 그대로 옮겨 적었기 때문이다.)
