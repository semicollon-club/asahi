...
Postgres도 소유자의 Claude 구독도 건드릴 수 없다. 봇과 워커 세션이 물리적으로 나뉘어 있던
...
- 지금 걱정해야 할 자격증명은 `DATABASE_URL`이 아니라 `WORKER_TOKEN`이다 —
  `docs/security/risk-register.md` §1·§2와 ADR 0005를 이 경계에 맞춰 갱신했다. ADR
  0005("소유자 전용 워커 위임")가 전제하던 메커니즘(작업 큐, 하트비트 `isOnline` 판정)은
  사라졌지만, "워커는 소유자만 붙일 수 있다"는 정책 자체는 새 메커니즘
  (`shouldConnectWorker`의 `isOwner && isPrivate`, `remoteToolHandler`의 `isOwnerDm`
  재확인)으로 그대로 이어진다 — 0005는 폐기가 아니라 갱신했다.
