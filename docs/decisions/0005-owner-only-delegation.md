---
status: Superseded
supersededBy: 0007-multi-worker-routing
lastReviewed: 2026-07-28
---

# 0005. 소유자 전용 워커 위임

> **대체됨(2026-07-28)**: 이 ADR의 결정("워커는 소유자만 붙을 수 있다")은
> [0007](./0007-multi-worker-routing.md)로 뒤집혔다 — 아래 "근거" 절이 전제로 삼던 것
> ("사용자별 토큰·행 단위 권한 분리 같은 인증 인프라가 아직 없다")이 `workers` 레지스트리
> 테이블(워커별 고유 토큰)로 채워지면서, 그 인프라가 없어서 손님을 막아야 했던 전제 자체가
> 사라졌다. 지금은 `role='allowed'`로 명시적으로 등록된 손님도 공유 워커에 연결된다. 아래
> 본문(과 바로 다음의 2026-07-26 갱신 노트)은 그 인프라가 없던 시점의 정당한 판단을 기록한
> 역사적 맥락으로 그대로 남긴다 — "지금" 코드가 무엇을 하는지의 서술이 아니다. 현재 정책과
> 그 근거는 0007 참고.
>
