...
(`agent/src/core/turnPrep.ts`)이 시스템 프롬프트 뒤에 붙일 블록 하나를 조립한다. 세 섹션은
건수 상한을 갖고, 기억 섹션은 문자 예산을 갖는다 — 예전에는 기억 섹션에 아무 제한도 없었다.
**다만 기억 섹션은 여전히 하드 상한이 없다**: 예산은 상한이 아니라 degrade 지점이고, 그 위로
제목 줄이 계속 얹힌다(아래 기억 항목에 실제 경계를 적었다). 의도한 선택이므로 "구멍이 막혔다"고
읽지 말 것.
...
- **`/기억정리`**(별칭 `/compact`, Claude Code의 `/compact`에 해당) — 지금 세션을 요약해
  ...
  도구·웹 검색·스킬·기억 쓰기(`remember`/`forget`/`character_fact`)를 닫고 돈다
