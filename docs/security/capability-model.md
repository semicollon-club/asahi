...
- **프록시 운영화(3단계, 2026-09-06)**: 프록시가 전달만 하지 않고 세 가지를 더 한다. (1) **모델 고정** — 작업 토큰이 못박은 모델과
  ...
  성공 응답 SSE 의 `usage`(message_start 의 입력·캐시, message_delta 의 누적 출력)를 곁으로 읽어 `llm_usage` 표에 모델 호출 하나마다
  한 행을 남긴다(부원·작업·모델·토큰 수·시각 — 프롬프트 본문·자격증명은 저장하지 않는다). (3) **부원별 창 상한** — 봇이 턴을 시작하기
