...
## 11. 손님 하네스 턴의 작업 폴더가 좁혀지지 않는다 (풀 하네스 4단계, 2026-09-06)

`runHarnessTurn`(`agent/src/core/agent.ts`)은 세션의 작업 폴더를 **워커 루트의 첫 번째 폴더**
(워크스페이스 루트)로 준다 — 신원과 무관하다. 프로필(`core/profiles.ts`)에는 폴더 스코프 필드가 없다.
설계 §6 은 손님을 `<루트>/<디스코드 userId>/` 로 좁히라고 하는데 그 코드가 아직 없다.

**지금은 노출되지 않는다**: 하네스는 소유자 전용이고, 소유자는 원래 관리자 스코프다. 하지만 5단계에서
손님 프로필을 여는 순간, 손님이 워크스페이스 루트에서 시작한다 — 다른 부원 폴더가 바로 옆에 보인다.
