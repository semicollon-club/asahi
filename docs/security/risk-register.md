...
## 10. 부원 세션 전사가 서로 가려지지 않는다 (풀 하네스 4단계, 2026-09-06)
...
**지금은 노출되지 않는다**: 하네스 턴은 소유자 전용이다(`decideHarnessDispatch` 가 `isOwner` 를 요구).
**부원을 하네스로 여는 5단계 전에** 셋 중 하나를 골라야 한다 — (가) 부원별 OS 계정·컨테이너로 돌아가
설계 결정을 뒤집는다, (나) 세션 전사를 부원별로 암호화하거나 턴이 끝나면 지운다(resume 을 포기하거나
봇이 대신 보관), (다) "부원의 세션 기록은 같은 기계의 다른 부원에게 비공개가 아니다"를 명시하고 연다.
고르지 않은 채 열면 사람들이 비공개라고 오해한 채 쓰게 된다.
