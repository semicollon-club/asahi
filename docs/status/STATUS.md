...
- **워커(개인·공유)** — 소유자 PC 또는 동아리 공용 미니PC에서 실행되며, 봇이 모델의 파일/셸
  ...
  이제 5분마다 `origin/main`을 확인해 새 커밋을 발견하면 센티넬 파일로 스스로 내려가고,
  작업 스케줄러의 "실패 시 다시 시작" 정책이 새 코드로 다시 띄운다(`deploy/update-worker.ps1`,
  `Stop`/`Start-ScheduledTask`를 부르지 않으므로 비관리자 계정 권한만으로 돈다). 워커가 어떤
  커밋으로 도는지는 소유자 DM의 `runtime_info`로 언제든 확인할 수 있고, 봇 커밋과 15분 넘게
  어긋나면(자동 갱신이 실제로 막혔다는 뜻이다) 소유자 DM으로 한 번 알림이 온다(같은 조합에는
  중복으로 오지 않는다).
