...
- **워커(개인·공유)** — 소유자 PC 또는 동아리 공용 미니PC에서 실행되며, 봇이 모델의 파일/셸
  ...
  `asahi-worker-update` 작업을 **등록하면** 5분마다 `origin/main`을 확인해 새 커밋을 발견할
  때 센티넬 파일로 스스로 내려가고, 작업 스케줄러의 "실패 시 다시 시작" 정책이 새 코드로
  다시 띄운다(`deploy/update-worker.ps1`, `Stop`/`Start-ScheduledTask`를 부르지 않으므로
  비관리자 계정 권한만으로 돈다). **이 등록은 미니PC 마다 사람이 한 번 해야 하는 수동
  절차라 병합 시점에 모든 미니PC 에 끝나 있는 것은 아니다**(등록 절차는
  `deploy/worker-셋업.md` "자동 갱신" 절 참고). 등록 후에는 워커가 어떤 커밋으로 도는지
  소유자 DM의 `runtime_info`로 언제든 확인할 수 있고, 봇 커밋과 15분 넘게 어긋나면(자동
