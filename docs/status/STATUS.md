...
- **정기 뉴스 게시** — 매일 KST 7시에 대회 소식·개발 뉴스를 조사해 각 채널에 올린다.
  ...
  알린다. 지정 채널이 없으면 친 곳으로 폴백한다. 일반 채널에서 예약어를 받아도 그 채널을 봇
  대화로 만들지 않으므로, 이후 그 채널의 잡담에 봇이 끼어들지 않는다. 게시 작업은 공개 채널
  계층으로 돌아 PC 도구가 열리지 않는다.
- **모델**: 기본 Opus 계열 최신 모델로 고정, 환경변수로 재정의 가능.

## 테스트

`agent/` 기준 vitest 38개 파일, **563개 통과 + 1개 skip**(564개). skip 1건은 Postgres READ ONLY
