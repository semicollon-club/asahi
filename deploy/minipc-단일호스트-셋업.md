...
저장한 뒤 **B 가 A 의 홈에 닿지 못하는지 확인한다.** 승격된 관리자 창에서:

```powershell
icacls C:\Users\asahi-bot
```

`NT AUTHORITY\SYSTEM`·`BUILTIN\Administrators`·`<컴퓨터>\asahi-bot`(그리고 `S-1-15-3-…` 앱컨테이너 SID) **넷만**
있어야 한다. `asahi` 나 `BUILTIN\Users` 줄이 보이면 격리가 없는 것이다 — 이 상태로 컷오버하면 부원이 셸 한 줄로
봇의 모든 자격증명을 읽고, `.env` 뿐 아니라 **봇 코드를 고쳐** 다음 재시작에 A 의 권한으로 돌릴 수 있다. 지우고
다시 확인한 뒤 진행한다:

```powershell
icacls C:\Users\asahi-bot /remove:g "<컴퓨터>\asahi"
```

- **B 의 창에서 `Get-Content …\.env` 로 확인하지 않는다.** 격리가 깨져 있으면 그 명령이 성공해 토큰 전부를 화면에
  뿌린다 — 확인하는 행위가 곧 유출이다. `icacls` 는 내용을 건드리지 않고 계정 B 를 거치지도 않는다.
- **한 번으로 끝내지 않는다.** 2026-09-07 에 이 항목이 실제로 열려 있는 채로 발견됐다(위험 등록부 §12) — 설정 때
  건너뛴 것인지 그 뒤에 바뀐 것인지 알 수 없었다. 부원을 열기 전 점검(ROADMAP "부원 오픈 게이트")에 다시 있다.
- 같은 명령을 기계의 다른 프로필(`C:\Users\<관리자>` 등)에도 한 번씩 돌려 `asahi` 줄이 없는지 본다.
...
- **소유자 DM 의 PC 작업**: (2026-09-17, ADR 0011 로 해소됨 — 소유자 DM 도 이 미니PC 의 공유 워커로 간다.) 아래는 그 전 기록이다: 소유자 DM 은 "그 소유자의 개인 워커"로 라우팅되는데, 개인 PC 워커는 루프백 허브에
